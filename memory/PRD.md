...
- **2026-07-27 (Trust-first hero + Founding Grower offer + Sales Playbook)**:
  - **Landing rewrite (P1) — user asked for a tighter, trust-first sales page**: Rewrote hero in `/app/backend/static/landing.html` — new headline "Kill the paperwork. Grow better birds." + Aussie-grower tagline + two clear CTAs (Founding Grower deal · 90-sec demo) + trust micro-line ("14-day trial · no card · cancel one-click · Live on Aussie farms"). Replaced generic "AU built" metric in the proof bar with more concrete signals ("Live: Ingham's · Baiada · BPL detected" + "5 min from signup to first reading"). Inserted a new **Founding Grower Deal** section (`#founding`) with gold scarcity badge, 4-benefit grid, and "18 of 25 spots left" counter.
  - **Sales Playbook created** at `/app/memory/sales_playbook.md`: 10-min demo script + objection cheat-sheet, 90-sec product-video outline (10 shots), outreach templates for 4 audiences (Growers · Integrators · Feed Mills · Equipment Suppliers), 18-column lead-list spec, 6-touch follow-up SOP, checkout-journey drop-off audit (P0s: kill Buyer Details Modal + fix magic-link handoff), Founding Grower case-study template with interview questions, weekly success metrics.
