...
- **2026-07-31 (Density NaN bug + Flock Forecast weigh-in display bug)**:
  - **P0 bug — user reported "NaN kg/m²" on the Density view**: After the per-shed weigh-in migration (2026-07-22), entries stored at `weighIns[groupId][age]` can be either legacy `number` OR the new `{ s1?, s2? }` object. `DensityView` (line 5335) and the Flock Forecast projection card (line 7707) were dividing that raw entry by 1000 → `{...}/1000 = NaN` → visible as "NaN kg/m²" and "NaN kg avg · day 14" on every shed. Fixed both call sites to use the `readWeighInGroup()` helper which handles both shapes. Verified visually — Density view no longer renders NaN.
  - **Not touched**: the underlying weigh-in data + per-shed slot storage all fine — this was purely a two-line display-layer bug.
