...
- **2026-07-31 (Success page redesign + resend-welcome endpoint + Density NaN fix)**:
  - **P0 conversion — magic-link "waiting" screen delivered**: Rewrote `/app/backend/static/success.html` to lead with a big **"YOU'RE IN — START RIGHT NOW"** gold CTA block that appears the instant Stripe confirms + provisioning finishes. The primary button is a **direct link to the reader/ops dashboard** with a "No email needed — this link is your direct entry" microcopy line, so growers don't wait or churn. Below it: a **"📧 We've also emailed you these links" block with a Resend button** (rate-limited: max 3 sends / session, 20s cooldown, live countdown after each send). Added a live status pill at the top ("Checking payment… → ✓ Payment confirmed"). Also renders per-farm rows with individual Field Reader links.
  - **New backend endpoint `POST /api/checkout/resend-welcome/{session_id}`**: Server-side handler that re-sends the welcome-email using the already-provisioned farm data (idempotent, rate-limited, logged to `payments_col.welcomeResends[]`). Uses `email_service.send_email`. Verified end-to-end: unknown session → 404 "Session not found"; endpoint properly registered under FastAPI (was initially attached to the wrong router — moved from `@api.post` to `@app.post`).
  - **P0 bug — Density NaN**: Fixed 2 spots (`DensityView` line 5335, Flock Forecast projection card line 7707) that divided the new `{ s1?, s2? }` weigh-in object by 1000. Both now go through `readWeighInGroup()` — no more NaN.
