...
- **2026-07-27 (Trust-first hero + Founding Grower offer + Sales Playbook + Live app screenshots)**:
  - **Landing rewrite (P1) — user asked for a tighter, trust-first sales page + real screenshots**: Rewrote hero with new headline "Kill the paperwork. Grow better birds." + Aussie-grower tagline + two CTAs (Founding Grower deal / 90-sec demo). Inserted new **Founding Grower Deal** section (`#founding`) with gold scarcity badge + 4-benefit grid + "18 of 25 spots left" counter. Added a **"See it in action"** section (`#how`) showcasing two REAL screenshots: (1) the desktop Feed Program with actual batch cascade (Shed 3&4, feed alloc/usage/on-hand for 80,000 birds), (2) the mobile Silo Reader inside a phone frame with silos A/B/C filled in + Farm Buddy alert. Screenshots captured live via headless Playwright and saved to both `/app/backend/static/` (source of truth for redeploys) and `/app/silo/artifacts/feed-program/dist/public/reader-assets/` (served by the frontend static server).
  - **Sales Playbook created** at `/app/memory/sales_playbook.md`: 10-min demo script + objection cheat-sheet, 90-sec product-video outline (10 shots), outreach templates for 4 audiences, lead-list template, 6-touch follow-up SOP, checkout-journey drop-off audit, Founding Grower case-study template, weekly success metrics.
  - **Not touched**: existing plans grid, Ops bundle builder, partner programs, FAQ, footer, checkout logic — all preserved so nothing regresses.
